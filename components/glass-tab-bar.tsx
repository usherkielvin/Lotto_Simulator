          ...
          <LinearGradient
            colors={
              isDark
                ...
                : ['rgba(255,255,255,0.80)', 'rgba(255,255,255,0)']
            }
            start={{ x: 0, y: 0 }}
            end={{ x: 0, y: 1 }}
...
export function GlassTabBar({ tabs, activeIndex, onTabPress }: GlassTabBarProps) {
  ...
  // translateY: negative = moves bar upward
  ...
  const pan = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_, g) => {
        ...
        // dy negative = dragging up = translateY should go more negative
        const next = lastY.current + g.dy;
        translateY.setValue(Math.min(0, Math.max(-maxLift, next)));
      },

      onPanResponderRelease: (_, g) => {
        const next = lastY.current + g.dy;
        const clamped = Math.min(0, Math.max(-maxLift, next));
        // snap back to bottom if released near default
  ...
  return (
    <Animated.View
      style={[styles.outer, { bottom: defaultBottom, transform: [{ translateY }] }]}
      {...pan.panHandlers}
    >
        ...
        <View style={styles.tabRow}>
          {tabs.map((tab, i) => (
            <TabButton
              ...
              onPress={() => { if (!didDrag.current) onTabPress(i); }}
              isDark={isDark}
            />
          ))}
        </View>
