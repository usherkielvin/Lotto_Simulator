...
function NumberBallInput({
  ...
  if (isDigitGame) {
    const is2D = game.id === '2d-ez2';
    const count = is2D ? 2 : game.id === '3d-swertres' ? 3 : game.id === '4digit' ? 4 : 6;
    const maxLen = is2D ? 2 : 1;
    ...
    const update = (idx: number, val: string) => {
      const cleaned = val.replace(/[^0-9]/g, '').slice(0, maxLen);
      const next = [...parts];
      next[idx] = cleaned;
      onChange(next.join(','));
      // Auto-advance when box is full
      ...
      <View style={{ marginTop: 10 }}>
        <View style={{ flexDirection: 'row', gap: 10, flexWrap: 'wrap' }}>
          {Array.from({ length: count }).map((_, i) => (
            <View key={i} style={[nb.digitWrap, { backgroundColor: p.stageBg, borderColor: parts[i] ? p.accent : p.cardBorder }]}>
              <TextInput
                ref={r => { refs.current[i] = r; }}
                style={[nb.digitInput, { color: p.textStrong, fontFamily: Fonts.mono }]}
                keyboardType="number-pad"
                maxLength={maxLen}
                value={parts[i]}
                onChangeText={v => update(i, v)}
                placeholder={is2D ? '00' : '0'}
                placeholderTextColor={p.textSoft}
                textAlign="center"
                returnKeyType={i < count - 1 ? 'next' : 'done'}
                onSubmitEditing={() => { if (i < count - 1) refs.current[i + 1]?.focus(); }}
              />
            </View>
          ))}
        </View>
  ...
  // Lotto games: text input + ball preview
  const balls = value.split(',').map(s => s.trim()).filter(Boolean);
  return (
    <View style={{ marginTop: 10 }}>
      <TextInput
        style={[{ borderWidth: 1, borderRadius: 12, paddingHorizontal: 12, paddingVertical: 12, fontSize: 15, fontFamily: Fonts.sans, backgroundColor: p.stageBg, color: p.textStrong, borderColor: p.cardBorder }]}
      ...
      {balls.length > 0 && (
        <View style={{ flexDirection: 'row', gap: 6, marginTop: 10, flexWrap: 'wrap' }}>
          {balls.map((n, i) => (
            <View key={i} style={[nb.ball, { backgroundColor: p.accent }]}>
              <Text style={[nb.ballText, { color: p.accentText }]}>{n}</Text>
            </View>
          ))}
        </View>
      )}
    ...
    <View style={[rc.row, { borderColor: p.cardBorder }]}>
      <View style={{ flex: 1 }}>
        <View style={rc.topRow}>
          ...
          <Text style={[rc.date, { color: p.textSoft }]}>{result.drawDateKey}</Text>
...
const rc = StyleSheet.create({
  row:      { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 10, borderTopWidth: 1, gap: 8 },
  topRow:   { flexDirection: 'row', alignItems: 'center', gap: 8, flexWrap: 'wrap' },
  game:     { fontSize: 13, fontWeight: '700', fontFamily: Fonts.sans },
  date:     { fontSize: 12, fontWeight: '500', fontFamily: Fonts.mono },
  badge:    { flexDirection: 'row', alignItems: 'center', gap: 2, paddingHorizontal: 6, paddingVertical: 2, borderRadius: 6 },
  badgeText:{ fontSize: 10, fontWeight: '700', fontFamily: Fonts.mono },
...
export default function AdminScreen() {
  ...
  const drawDateKey = drawDate.toISOString().slice(0, 10);
    ...
    <SafeAreaView style={[s.root, { backgroundColor: p.screenBg }]}>
      <ScrollView contentContainerStyle={s.scroll}>
        ...
        <View style={s.headerRow}>
          <Text style={[s.title, { color: p.textStrong }]}>Results</Text>
          <View style={s.headerBtns}>
            <Pressable style={[s.actionBtn, { backgroundColor: p.chipIdle }]} onPress={() => setImportVisible(true)}>
              <Ionicons name="clipboard-outline" size={14} color={p.chipIdleText} />
              <Text style={[s.actionBtnText, { color: p.chipIdleText }]}>Import</Text>
            </Pressable>
            <Pressable style={[s.addBtn, { backgroundColor: p.accent }]} onPress={openAdd}>
              <Ionicons name="add" size={18} color={p.accentText} />
              <Text style={[s.addBtnText, { color: p.accentText }]}>Add</Text>
            </Pressable>
          </View>
        </View>
              ...
              return (
                <Pressable key={cat}
                  ...
                  <Text style={[s.filterTabText, { color: active ? p.accentText : p.chipIdleText, textAlign: 'center' }]}>
                    {cat === 'all' ? 'All Results' : cat === 'lotto' ? 'Lotto Games' : 'Digit Games'}
                  </Text>
                </Pressable>
              );
            })}
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 8 }}>
            <View style={s.filterRow}>
              <Pressable
                ...
                <Text style={[s.subFilterTabText, { color: filterGameId === 'all' ? p.chipActiveText : p.textSoft }]}>All {filterCategory !== 'all' ? filterCategory : ''}</Text>
              </Pressable>
        ...
        <View style={[s.card, { backgroundColor: p.cardBg, borderColor: p.cardBorder }]}>
          <View style={s.cardHead}>
            <Text style={[s.sectionTitle, { color: p.textStrong }]}>
              {filterGameId === 'all' ? 'All Results' : gameName(filterGameId)}
            </Text>
            <Text style={[s.countBadge, { color: p.textSoft }]}>{filtered.length} entries</Text>
          </View>
          ...
          {loadingResults ? (
            ...
            <View style={{ paddingVertical: 30, alignItems: 'center' }}>
              <Ionicons name="search-outline" size={32} color={p.textSoft} />
              <Text style={[s.emptyText, { color: p.textSoft, marginTop: 12 }]}>
                {filterGameId === 'all' ? 'No results found.' : `No ${gameName(filterGameId)} results yet.`}
              </Text>
            </View>
          ) : (
            filtered.map(r => (
        ...
        >
          ...
          <View style={[s.sheet, { backgroundColor: p.cardBg }]}>
            ...
            >
              ...
              <Pressable style={[s.btn, { backgroundColor: p.accent, opacity: saving ? 0.7 : 1, marginTop: 24 }]}
              ...
              <Pressable style={[s.btn, { backgroundColor: p.chipIdle, marginTop: 10 }]} onPress={() => setModalVisible(false)}>
...
const s = StyleSheet.create({
  ...
  headerRow:      { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4, gap: 10, flexWrap: 'wrap' },
  ...
  actionBtn:      { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 8, borderRadius: 10 },
  actionBtnText:  { fontSize: 13, fontWeight: '700', fontFamily: Fonts.sans },
  addBtn:         { flexDirection: 'row', alignItems: 'center', gap: 4, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 10 },
  ...
  filterContainer:{ gap: 4, marginBottom: 8 },
  ...
  sectionTitle:   { fontSize: 16, fontWeight: '800', fontFamily: Fonts.rounded },
  countBadge:     { fontSize: 12, fontWeight: '600', fontFamily: Fonts.mono },
