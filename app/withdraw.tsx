  ...
  return (
    <SafeAreaView style={[s.root, { backgroundColor: p.screenBg }]}>
      ...
      <View style={s.header}>
        ...
        <Text style={[s.headerTitle, { color: p.textStrong }]}>Withdraw</Text>
        <View style={{ width: 38 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={[s.iconWrap, { backgroundColor: '#fee2e2' }]}>
          <Ionicons name="arrow-up-circle-outline" size={40} color="#dc2626" />
        </View>
        <Text style={[s.title, { color: p.textStrong }]}>Withdraw Credits</Text>
        <Text style={[s.sub, { color: p.textSoft }]}>Minimum withdrawal is ₱50</Text>

        {/* Current balance */}
        {balance !== null && (
          <View style={[s.balCard, { backgroundColor: p.cardBg, borderColor: p.cardBorder }]}>
            <Text style={[s.balLabel, { color: p.textSoft }]}>{newBal !== null ? 'New Balance' : 'Available Balance'}</Text>
            <Text style={[s.balValue, { color: newBal !== null ? '#dc2626' : p.textStrong }]}>
              {formatPHP(newBal ?? balance)}
            </Text>
          </View>
        )}
